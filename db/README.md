...
`docker compose up` gives you a working product: `lms-db` creates the
...
By default these talk to the `lms-db` container through
...
```bash
DATABASE_URL=postgres://user:pass@host:5432/academy npm run demo:status
```
