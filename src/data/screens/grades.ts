 ...
 *
 * Translation. A gradebook row's `name` is the assignment's own title and is
 * demo fiction; its `kind` is the column's vocabulary and is not, so the kinds
 * are ids here with `data.gradeKind.*` as their reader-facing names. The
 * certificate card's copy is all module-level `const`s — evaluated before
 * React exists, so they cannot be getters — and lives under `data.grades.*`.
 */
