 ...
 *
 * Several places need `t()` / `money()` / the active locale and cannot call a
 * hook to get them: the zustand store (toast copy is produced inside actions),
 * `lib/schedule.ts` and `lib/exam.ts` (pure functions the tests call with no
 * React at all), and anything else that runs before React does. Rather than
 * duplicate the runtime, `<App>` pushes the provider's own `t` / `money` /
 * `number` in here on every render — so these forward to exactly the functions
 * the tree is using, and there is still one lookup table and one set of `Intl`
 * rules in the app.
 *
...
/**
 * Lookup for keys assembled at runtime from data (`'level.' + slug`), where the
 * compiler cannot check the key. Returns `fallback` — the raw English from the
 * seed — when the bundle has nothing for it, so unknown catalogue data still
